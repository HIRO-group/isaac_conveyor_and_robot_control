...
| `src/conveyor_indexing/mcap_recorder.py` | `McapRecorder` - episode-free full MCAP capture, camera/robot/PLC channels plus sim-only ground truth (see "Full MCAP capture" below). |
...
Set `CONVEYOR_INDEXING_RECORD_MCAP=1` to additionally (independent of
`CONVEYOR_INDEXING_RECORD` above - either, both, or neither) record every
raw channel to `data/mcap/`, with no episode concept: camera frames
(`foxglove.RawImage`, 30Hz), per-arm joint/DIO state (`theia.robot.v1.PositionStatus`,
120Hz), PLC conveyor state (`theia.plc_connector.v1.StateConveyors`, 120Hz) -
mirroring theia's real data-collection service's wire format field-for-field
(schemas generated by `gen_proto.sh` from theia's own `robot.proto` /
`foxglove/raw_image.proto`) - plus sim-only ground truth with no real-robot
equivalent (`proto/sim_state.proto`): every active box's pose/velocity/type
(`BoxStates`, 120Hz), spawn/despawn events (`BoxEvent`), each arm's
pick-and-place phase transitions (`ArmPhaseTransition`), and a `RunMetadata`
message (spawn seed, physics/camera rates, camera extrinsics, git sha)
written once per file. The goal is a single capture that can fully reproduce
the run, not just what a real robot's sensors would see. Full schema/writer:
`src/conveyor_indexing/mcap_recorder.py`.

Files rotate every ~30 sim-seconds, named `<start_ns>_INCOMPLETE.mcap` while
open and `<start_ns>_<end_ns>.mcap` once closed - same convention as theia's
real service, so a downstream reader can skip in-progress files the same way.
Same drop-and-warn-rather-than-stall queue as the parquet recorder above.

### Batch/parallel-instance env vars

These support running many sim instances in parallel (e.g. one per Vertex AI
job - see `~/theia/model_training/sim_collection/`) without them colliding on
output paths, episode IDs, or random seeds:

| Env var | Default | Purpose |
|---|---|---|
| `CONVEYOR_INDEXING_DATA_DIR` | `<repo>/data` | Overrides where recordings/mcap/tick-log all land - point each instance at its own directory |
| `CONVEYOR_INDEXING_MAX_SIM_SECONDS` | unset (unlimited) | Sim-time cap; reaching it triggers the same clean-shutdown path as SIGTERM |
| `CONVEYOR_INDEXING_EPISODE_ID_BASE` | `0` | Starting value for the episode counter - set to a distinct value per instance (e.g. `instance_index * 10_000_000`) so IDs stay globally unique when a conversion pass walks multiple instances' output together |
| `CONVEYOR_INDEXING_INSTANCE_INDEX` | `0` | Recorded into MCAP `RunMetadata` only - purely descriptive |
| `CONVEYOR_INDEXING_RECORD_QUEUE` | `60` | Parquet recorder's queue depth - raise for headroom if the sim runs meaningfully faster than the ~0.23x realtime measured on the dev L4 |
| `CONVEYOR_INDEXING_SPAWN_SEED` | random | Already existed - **must differ per instance**, or every instance spawns identical box waves |
