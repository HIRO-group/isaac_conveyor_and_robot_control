"""Episode-free MCAP recorder: the "record everything" base capture an
external dc_to_lerobot.py-style converter (and future tooling) can convert
from, with no episode concept baked in - see the top-level README's
"Design" section and sim_cell.recording for the CONVEYOR_INDEXING_RECORD_MCAP
gate.
...
two channel families:
  ...
  Fixed-schema channels (generated by proto/gen_proto.sh, byte-compatible
  with an external downstream toolchain's wire format so the same converter
  routes sim and non-sim captures identically by schema name):
    foxglove.RawImage              sim/camera/<serial>/color          30Hz/camera
    robot.v1.PositionStatus        sim/robot/arm<n>/position_status   120Hz/arm
    robot.v1.MoveTarget            sim/robot/arm<n>/move_target       once/arm (see below)
    plc_connector.v1.StateConveyors  sim/plc/state_conveyors          120Hz

  Sim-only ground truth (conveyor_indexing.v1, proto/sim_state.proto - no
  external equivalent; exists so a capture can fully reproduce the run, not
...
MoveTarget is a per-move message on the external robot schema, but this
sim's cuMotion planner has no equivalent per-tick "target" to mirror - it
plans whole trajectories, not waypoint commands. The downstream converter's
replay gate only checks that at least one MoveTarget with a non-empty pose
has ever been seen (`if mt.pose_target.pose: move_target_seen = True`, never
reset), so one placeholder message per arm at recorder construction
satisfies the gate structurally without pretending to be a real motion
command - see _write_move_target_stub.
...
INCOMPLETE-skip rule) the external downstream converter already uses.
...
# The real image encoding is uppercase ("RGB8", see cameras.specs.COLOR_FORMAT);
# foxglove's own RawImage examples use lowercase - either is fine since every
# consumer (this repo's, the downstream converter's) only checks "BGR" in .upper().
...
class McapRecorder:
    ...
    def record_camera_frame(
        ...
        # a name) - matches the external data-collection schema exactly, so
        # the downstream converter's camera_role_from_frame_id() parses it unchanged.
    ...
    def record_position_status(
        ...
        self._enqueue(f"sim/robot/arm{arm}/position_status", msg, sim_time_s)
    ...
    def record_state_conveyors(self, sim_time_s: float, state_msg: plc.StateConveyors) -> None:
        ...
        self._enqueue("sim/plc/state_conveyors", state_msg, sim_time_s)
    ...
    def write_move_target_stub(self, arm: int) -> None:
        ...
        self._enqueue(f"sim/robot/arm{arm}/move_target", msg, 0.0)
